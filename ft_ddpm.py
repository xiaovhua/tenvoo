...
if __name__ == '__main__':
    ...
    if args.dataset_name == 'brats':
        ...
        if os.path.exists(os.path.join(args.dataset_root, 'clean_brats.txt')): # filter brats mri scans
            ...
            mri_list = [r for r in glob.glob(args.dataset_root) if r.replace('\\', '/').split('/')[-1] in clean_files]
        else:
            mri_list = [r for r in glob.glob(args.dataset_root)]
        dataset = DDPMDataset(root=mri_list, transform=default_transform(spatial_size=args.mri_resolution, scale_ratio=args.scale_ratio))
    else:
        args.dataset_root = os.path.join(args.dataset_root, *GLOB_PATHS[args.dataset_name])
