...
class Linear(TenVOOLayer, nn.Linear):
    ...
    def cal_quanta_weights(self):
        return F.pad(
            self.einsum_expr_eval(*[self.quanta_weights[f'{dim1} {dim2}'].to(self.weight.dtype) for (dim1, dim2) in \
                                    itertools.combinations(range(-1, -self.d_in - 1, -1), 2)]
                                  ).reshape(self.total_features2, self.total_features),
            (0, self.in_features - self.total_features, 0, self.out_features - self.total_features2),
            'constant',
            0.
        )

    def cal_quanta_weights2(self):
        return F.pad(
            self.einsum_expr_eval(*[self.quanta_weights2[f'{dim1} {dim2}'].to(self.weight.dtype) for (dim1, dim2) in \
                                    itertools.combinations(range(-1, -self.d_in - 1, -1), 2)]
                                  ).reshape(self.total_features2, self.total_features),
            (0, self.in_features - self.total_features, 0, self.out_features - self.total_features2),
            'constant',
    ...
    def forward_quanta_weights(self, x, quanta_weights):
        ...
        """
        return self.einsum_expr_train(
            x,
            *[quanta_weights[f'{dim1} {dim2}'].to(x.dtype) for (dim1, dim2) in itertools.combinations(range(-1, -self.d_in - 1, -1), 2)]
        )
    ...
    def train(self, mode: bool = True):
        ...
        if mode:
            if self.merge_weights and self.merged:
                ...
                if self.d_in > 0:
                    full_quanta_weights = self.cal_quanta_weights()
    ...
    def merge_frozen_weights(self):
        ...
        if self.frozen_merged:
            ...
            full_quanta_weights = -self.cal_quanta_weights2()
            self.weight.data += T(full_quanta_weights)
...
class Conv3d(TenVOOLayer, nn.Conv3d):
    ...
    def cal_tenvoo_weights2(self):
        return self.einsum_expr(*[self.tenvoo_weights2[str(ti)].to(self.weight.dtype) for ti in range(len(self.tenvoo_weights))])
    ...
    def merge_frozen_weights(self):
        if self.frozen_merged:
            ...
            full_tenvoo_weights = -self.cal_tenvoo_weights2()
            Cout, Cin, kD, kH, kW = self.weight.shape
